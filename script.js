...
let clickTimeout = null; // Timeout variable for click event
...
document.getElementById("voiceToTextBTN").addEventListener("click", function(event) {
    ...
    // Set a timeout to delay single-click action
    ...
    }, 250); // Delay the single click action by 250ms
});

document.getElementById("voiceToTextBTN").addEventListener("dblclick", function(event) {
    event.preventDefault();
    ...
    // Perform the double-click action (which closes the interface if open)
    const userSpace = document.getElementById("userspace");
